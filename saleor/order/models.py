...
class OrderQueryset(models.QuerySet):
    ...
    def fullfilled(self):
        return self.filter(status=OrderStatus.FULFILLED)
...
class Order(ModelWithMetadata):
    ...
    riderid = models.ForeignKey(
        Rider, related_name="rider", null=True, on_delete=models.SET_NULL,blank=True
    )

    shopid = models.ForeignKey(
        SubShop, related_name="order", null=True, on_delete=models.SET_NULL,blank=True
    )
    ...
    class Meta:
        ...
        permissions = (
            ...
            (
                "assign_order",
                pgettext_lazy("Permission description", "Assign order."),
            ),
        )
    ...
    def get_customer_phone(self):
        return self.user.phone if self.user else self.user_email
