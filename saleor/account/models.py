...
class Address(models.Model):
    ...
    postal_code = models.CharField(max_length=20, blank=True,default="52250")
...
class UserManager(BaseUserManager):
    ...
    def verify(self, sms_code, phone):
        user = self.get_queryset().get(phone=phone, sms_code=sms_code)
    ...
    def create_superuser(self, phone, password=None, **extra_fields):
        return self.create_user(
            phone, password, is_staff=True, is_superuser=True,phone_verified=True, **extra_fields
        )
...
class User(PermissionsMixin, ModelWithMetadata, AbstractBaseUser):
    ...
    first_name = models.CharField(max_length=256, blank=True)
    last_name = models.CharField(max_length=256, blank=True)
    phone = models.CharField(max_length=12, blank=False,default=False, unique=True)
    ...
    shopid = models.OneToOneField(
        SubShop, related_name="admin_subshop", null=True, on_delete=models.SET_NULL,blank=True
    )
    riderid = models.OneToOneField(
        Rider, related_name="admin_rider", null=True, on_delete=models.CASCADE,blank=True
    )
