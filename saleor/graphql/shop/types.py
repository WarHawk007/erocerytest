...
from ..core.enums import WeightUnitsEnum, ReportingPeriod
...
class SubShopOrderFilterInput(FilterInputObjectType):
    class Meta:
        filterset_class = OrderFilter


class SubShop(MetadataObjectType, CountableDjangoObjectType):
    ...
    orders = FilterInputConnectionField(
        ...
        filter=SubShopOrderFilterInput(description="Filtering options for orders."),
    ...
    riders = gql_optimizer.field(
        PrefetchingConnectionField(
            "saleor.graphql.rider.types.Rider", description="List of user's Shop orders."
        ),
        model_field="riders"
    )

    @staticmethod
    def resolve_orders(root: subshop_models.SubShop, info, **_kwargs):
        return root.order.all()

    @staticmethod
    def resolve_riders(root: subshop_models.SubShop, info, **_kwargs):
        temp_riders = root.rider_subshop.all()
        riders = [rider for rider in temp_riders if rider]
        return riders 

    class Meta:
        description = "Represents an order in the Sub Shop."
