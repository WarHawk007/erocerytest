...
class Order(MetadataObjectType, CountableDjangoObjectType):
    user = graphene.Field(User, description="User who performed the action.")
    subshop = graphene.Field(SubShop, description="Order of SubShop")
    rider = graphene.Field(Rider, description="Order Assigned to Rider")
    created = graphene.String(description="Order creation date")
    ...
    @staticmethod
    def resolve_subshop(root: models.OrderEvent, info):
        return root.shopid
    ...
    def resolve_user(root: models.Order, info):
        user = info.context.user
        if user == root.user or user.has_perm("account.manage_users") or user.has_perm("account.view_user"):
            return root.user
        raise PermissionDenied()
