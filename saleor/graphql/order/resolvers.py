...
def resolve_orders(info, created, status, query):
    ...
    if user.shopid:
        qs = user.shopid.order.confirmed()
    else:
...
def resolve_orders_total(_info, period):
    ...
    if user.shopid:
        qs = user.shopid.order.confirmed().exclude(status=OrderStatus.CANCELED)
    else:
...
def resolve_homepage_events(info):
    ...
    user = info.context.user
    if user.shopid:
        orders = user.shopid.order.confirmed()
        return OrderEvent.objects.filter(type__in=types,order__id__in=orders)
    else:
