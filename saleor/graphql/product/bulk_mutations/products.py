...
from .bulk_products import bulk_product
...
class ProductBulkCreateInput(graphene.InputObjectType):
    productcsv = Upload(
        required=True, description="Represents an CSV file in a multipart request."
    )
...
class ProductBulkCreate(BaseMutation):
    ...
    class Arguments:
        input = ProductBulkCreateInput(
            required=True, description="Fields required to create bulk products."
        )

    class Meta:
        description = (
            "Create Bulk Products From csv File. This mutation must be sent as a `multipart` "
    ...
    def perform_mutation(cls, _root, info, **data):
        data = data.get("input")
        product_csv = info.context.FILES.get(data["productcsv"])
        if not product_csv:
            raise ValidationError({"productcsv":"File Required"})
        message = bulk_product(product_csv)
        return ProductBulkCreate(message=message)
