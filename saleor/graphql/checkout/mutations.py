...
from ...core import analytics, sms
...
from ..account.validation import isPhoneNumber
...
class CheckoutCreateInput(graphene.InputObjectType):
    ...
    phone = graphene.String(description="The customer's Phone.")
...
class CheckoutCreate(ModelMutation, I18nMixin):
    ...
    def clean_input(cls, info, instance: models.Checkout, data):
        ...
        if user.is_authenticated:
            ...
            cleaned_input["phone"] = phone or user.phone
        print(cleaned_input)
...
class CheckoutEmailUpdate(BaseMutation):
    ...
    class Arguments:
        ...
        phone = graphene.String(required=True, description="Phone.")

    class Meta:
        description = "Updates Phone in the existing checkout object."
        error_type_class = CheckoutError
        error_type_field = "checkout_errors"

    @classmethod
    def perform_mutation(cls, _root, info, checkout_id, phone):
        checkout = cls.get_node_or_error(
            info, checkout_id, only_type=Checkout, field="checkout_id"
        )
        ...
        cls.clean_instance(checkout)
        checkout.save(update_fields=["phone"])
...
class CheckoutComplete(BaseMutation):
    ...
    def perform_mutation(cls, _root, info, checkout_id, store_source):
        ...
        # get id of the city shop
        shopid = subshop_models.SubShop.objects.get(city=shipping_address.city)
        order_data["shopid"] = shopid
